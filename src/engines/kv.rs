...
use crate::kvfile::{KVFile, MAGIC_KV, KVRecordKind, KVRecord};
...
pub struct KvStore {
    ...
    handles: HashMap<file_id, KVFile>,
...
impl KvStore {
    ...
    fn handle_opened_file(&mut self, fid: file_id, file: Box<dyn Storage>) -> Result<()> {
        ...
        match h.magic() {
            MAGIC_KV => {
                let kv_file = KVFile::open(file, false)?;
    ...
    fn new_writable_kv_file(&mut self) -> Result<()> {
        ...
        self.handles.insert(fid, KVFile::open(handle, true)?);
    ...
    fn maybe_compact(&mut self) -> Result<()> {
        ...
        let mut current_file = KVFile::open(temp_file, true)?;
            ...
            match current_file.append_record(&record)
            {
                ...
                Err(err) => {
                    if let Some(KvError::FileSizeExceed) = err.downcast_ref::<KvError>() {
                        ...
                        current_file = KVFile::open(temp_file, true)?;
