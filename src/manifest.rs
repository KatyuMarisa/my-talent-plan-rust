...
mod manifest_unit_tests {
    use crate::{Result, kvfile::{KVFile, KVRecord}, manifest::file_id};
    ...
    fn simple_test() -> Result<()> {
        ...
        let mut kv_file_1 = KVFile::open(file_1, true)?;
        let mut kv_file_2 = KVFile::open(file_2, true)?;
        let mut kv_file_3 = KVFile::open(file_3, true)?;
        ...
        for (fid, file) in files.drain(..) {
            if fid == 1 {
                kv_file_1 = KVFile::open(file, false)?;
            } else {
                kv_file_3 = KVFile::open(file, false)?;
            }
        }
        ...
        let mut kv_file_4 = KVFile::open(file_4, true)?;
